...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
  (define (gen-list total max-pieces max-value p-list)
    (cond ((= max-value 0) ())
        ((= total 0) (if (<= (length p-list) max-pieces) 
                   (cons p-list ())
                   ()))
        ((< total 0) ())
        (else (append (gen-list (- total max-value) max-pieces max-value (append p-list (list max-value)))
                (gen-list total max-pieces (- max-value 1) p-list)))))
  (gen-list total max-pieces max-value ()))
...
(define (tree-sums tree)
  ...
  (define (tree-sum-leaves tree sum tlist)
    (if (leaf? tree)
      (cons (+ sum (entry tree)) tlist)
      (tree-sum-branches (children tree) (+ sum (entry tree)) tlist)))
  (define (tree-sum-branches tree sum tlist)
    (if (null? tree)
      ()
      (append (tree-sum-leaves (entry tree) sum tlist)
          (tree-sum-branches (children tree) sum tlist))))
  (tree-sum-leaves tree 0 ()))
