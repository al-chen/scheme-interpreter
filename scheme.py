...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        lf = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, lf)
    elif isinstance(procedure, MuProcedure):
        "*** YOUR CODE HERE ***"
        lf = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, lf)
    else:
...
def apply_primitive(procedure, args, env):
    ...
    if procedure.use_env:
        lst.append(env)
    try:
        result = procedure.fn(*lst)
    except TypeError as e:
        raise SchemeError("TypeError {0}".format(str(e)))
    return result
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            ...
            raise SchemeError("too many or too few arguments are given {0} {1}".format(str(formals), str(vals)))
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target[0]):
            lf = do_lambda_form(Pair(target.second, vals.second), env)
            env.define(target[0], lf)
            return target[0]
        else:
...
def do_let_form(vals, env):
    ...
    #print(len(vals))
    for i in bindings:
        names = Pair(i[0], names)
        values = Pair(scheme_eval(i[1], env), values)
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            ...
            if clause.second == nil:
                ##### Resolve tests.scm line 553/581 issue
                if isinstance(test, bool):
                    return True
                elif isinstance(test, Pair):
                    return test
                else:
                    return clause.first
            else:
                target = clause.second
                target_len = len(clause.second)
                if target_len == 0: 
                    return test
                target = Pair('begin', target)
                return target
    return okay
...
def check_formals(formals):
    ...
    for i in formals:
        if not scheme_symbolp(i):
            raise SchemeError("bad parameter: {0}".format(str(i)))
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            if isinstance(procedure, PrimitiveProcedure):
                ...
                "*** YOUR CODE HERE ***"
                expr, env = procedure.body, procedure.env.make_call_frame(procedure.formals, args)
            elif isinstance(procedure, MuProcedure):
                "*** YOUR CODE HERE ***"
